  ...
  const fetchRoles = async () => {
    ...
    if (response.ok) {
      const roleHierarchy = {
        ...
        'Finance Staff': 5,
        'Kitchen Manager': 6,
        'Kitchen Team Leader': 7,
        'Kitchen Staff': 8,
      };
      const allRoles = response.data.role;
      const currentUserRole = user.role; 
      const filteredRoles = allRoles.filter(role => roleHierarchy[role] >= roleHierarchy[currentUserRole]);
        ...
        <Autocomplete
          ...
          getOptionDisabled={(categories) =>
            selectedCategories.some(
              (chosenCategories) => chosenCategories.id === categories.id
            )
          }
